...
```
[ Payload original (IP: LAN→LAN) ]
        ↓ GRE encapsula
[ GRE Header | IP original ]
        ↓ IPSec (ESP modo transport) cifra
[ ESP | GRE cifrado | IP outer (WAN) ]
```
...
**Dato clave para el video** — en `show crypto ipsec sa` los selectores de tráfico son:
```
local  ident: (10.11.85.65/255.255.255.255/47/0)   ← proto 47 = GRE
remote ident: (10.11.85.130/255.255.255.255/47/0)
```
Esto confirma que IPSec está protegiendo el protocolo GRE (47), no las IPs de las LANs directamente.

---

## Comparativa de las 3 variantes IKEv1

| Característica        | Policy-Based          | Route-Based (VTI)       | GRE over IPSec          |
|-----------------------|-----------------------|-------------------------|-------------------------|
| Interfaz de túnel     | No existe             | Tunnel0 (ipsec ipv4)    | Tunnel0 (gre ip)        |
| Modo IPSec            | Tunnel                | Tunnel                  | **Transport**           |
| Crypto Map            | En e0/0               | No se usa               | En e0/0                 |
| IPSec Profile         | No se usa             | En Tunnel0              | No se usa               |
| ACL tráfico           | LAN ↔ LAN             | No se usa               | `permit gre host host`  |
| Soporte multicast     | No                    | No (nativo)             | **Sí** (GRE lo permite) |
| Enrutamiento dinámico | No                    | Soportado               | **Soportado** (mejor)   |
| Overhead              | Menor                 | Menor                   | Mayor (doble header)    |
...
|---------------------------|-----------------------------------------------------------|
| Interceptación            | AES-256 cifra el payload GRE completo                    |
| Modificación de paquetes  | HMAC-SHA-256 verifica integridad del paquete GRE         |
| Replay attacks            | Anti-replay de ESP habilitado por defecto                |
| GRE sin cifrar            | Sin crypto map en e0/0, el GRE viaja en claro → aplicar siempre |
| Acceso no autorizado      | PSK + DH Group 14 para levantar la SA IKE                |
